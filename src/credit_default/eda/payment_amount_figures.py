...
from credit_default.eda.payment_amount import CHRONOLOGICAL_PAY_COLS, MONTH_MAPPING, compute_histogram_stats, compute_histogram_percentages
...
def plot_six_panel_distributions(df: pd.DataFrame, disp: pd.DataFrame, out_dir: Path) -> Path:
    ...
    for idx, col in enumerate(CHRONOLOGICAL_PAY_COLS):
        ax = axes[idx]
        ...
        ax.set_ylabel("Count")
        if idx >= 3:
            ax.set_xlabel("Raw PAY_AMT — display range only")
        ax.spines[["top", "right"]].set_visible(False)
        
    fig.suptitle("PAY_AMT Distributions", fontsize=14, y=0.98)
...
def plot_six_panel_distributions_by_target(df: pd.DataFrame, disp: pd.DataFrame, out_dir: Path) -> Path:
    ...
    fig.suptitle("PAY_AMT Distributions by Target", fontsize=14, y=0.98)
    _plot_display_range_disclosure(fig)
    fig.tight_layout(rect=[0, 0.05, 1, 0.95])
...
def plot_six_panel_boxplots(df: pd.DataFrame, out_dir: Path) -> Path:
    ...
    fig.suptitle("PAY_AMT Boxplots by Target and Month", fontsize=14, y=0.98)
...
def plot_sign_proportions(sign_over: pd.DataFrame, out_dir: Path) -> Path:
    ...
    ax.set_title("Proportion of Negative, Zero, and Positive Payment Amounts", fontsize=12)
...
def plot_sign_default_rates(sign_over: pd.DataFrame, out_dir: Path) -> Path:
    ...
    needs_note = False
    plotted_any_negative = False
    for sign in ["negative", "zero", "positive"]:
        sub = sign_over[sign_over["sign_category"] == sign]
        
        # Do not plot if category has no observed combinations in every month
        if not sub["observed_combination"].any():
            continue
            
        if sign == "negative":
            plotted_any_negative = True
    ...
    if not plotted_any_negative:
        fig.text(0.5, 0.01, "No negative PAY_AMT observations were present in the raw dataset.", ha="center", fontsize=9, color="red")
...
def plot_adjacent_change_distributions(df: pd.DataFrame, out_dir: Path) -> Path:
    ...
    pairs = [
        ...
        ("July->Aug", "PAY_AMT3", "PAY_AMT2"),
        ("Aug->Sep", "PAY_AMT2", "PAY_AMT1")
    ]
...
def plot_correlation_heatmap(corr: pd.DataFrame, out_dir: Path) -> Path:
    ...
    ax.set_title("Six-Month PAY_AMT Pearson Correlation Matrix", fontsize=12)
